...
router.post('/reply', async (req, res) => {
  ...
  try {
    const threadRes = await db.query(`
      ...
      FROM inbox_threads t 
      JOIN email_accounts e ON t.account_id = e.id 
      WHERE t.id = $1`, [thread_id]);
      
    if (threadRes.rowCount === 0) return res.status(404).json({error: 'Thread not found'});
    
    const thread = threadRes.rows[0];
    
    const lastMsgRes = await db.query(`SELECT message_id, subject FROM inbox_messages WHERE thread_id = $1 AND direction = 'INBOUND' ORDER BY date DESC LIMIT 1`, [thread_id]);
